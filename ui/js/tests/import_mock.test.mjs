 ...
 * boundary, cut a new take onto a committed slot, and grow a stack a
 * clip child named after the file. Plus the pure drop-gesture pieces
...
import { getLastImport, resetLastImport, DIALOG_PATH, nearestWholeQ }
    from '../mock/import.js';
...
test('a first take lands on the nearest whole Q to atQ, hysteresis-snapped', async () => {
    loadScenario('single-clip');            // Q = 2 s (the sole clip defines it)
    resetLastImport();
    const Q = 2 * MOCK_Q;
    const id = await emptyClip();
    // 1.5 Q rounds to 2 Q; the file is 2.1 Q long → snaps to 2 Q.
    const path = '/audio/pad.wav#len=' + Math.round(2.1 * Q);
    assert.equal(await callNative('importAudio', id, path, [3, 2]), true);
    const n = findNode(id);
    assert.equal(n.origin, state.islandEpoch + 2 * Q);
    assert.equal(n.duration, 2 * Q);
    assert.equal(n.takes.length, 1);
    assert.deepEqual(getLastImport(),
        { uuid: id, path, atQ: [3, 2], form: 'first', targetId: id });
});

test('a pre-Q import defines Q from the file length', async () => {
    loadScenario('empty');
    resetLastImport();
    const id = await emptyClip();
    const len = 3 * MOCK_Q;
    assert.equal(await callNative('importAudio', id, '/audio/seed.wav#len=' + len, [0, 1]), true);
...
test('a committed slot takes a NEW TAKE, active on arrival', async () => {
    ...
    assert.equal(await callNative('importAudio', 'clip-1', '/audio/alt.wav', [0, 1]), true);
...
test('a stack target gains a clip child named after the file', async () => {
    loadScenario('stack-with-clips');
    resetLastImport();
    assert.equal(await callNative('importAudio', 'stack-1', '/audio/Vocal Take.wav', [1, 1]), true);
...
test('the dialog verb records the dialog placeholder path', async () => {
    ...
    resetLastImport();
    assert.equal(await callNative('importAudioWithDialog', 'mock-root', [0, 1]), true);
...
test('a live take refuses both verbs and records nothing', async () => {
    ...
    assert.equal(await callNative('importAudio', 'clip-1', '/audio/x.wav', [0, 1]), false);
    assert.equal(await callNative('importAudioWithDialog', 'clip-1', [0, 1]), false);
...
test('a MIDI track and an unreadable file refuse', async () => {
    ...
    resetLastImport();
    assert.equal(await callNative('importAudio', 'midi-1', '/audio/x.wav', [0, 1]), false);
    assert.equal(await callNative('importAudio', 'seed', '/audio/x.missing', [0, 1]), false);
    assert.equal(getLastImport(), null);
});

test('nearestWholeQ rounds a QTime or a bare number', () => {
    assert.equal(nearestWholeQ([3, 2]), 2);
    assert.equal(nearestWholeQ([1, 4]), 0);
    assert.equal(nearestWholeQ(2.4), 2);
    assert.equal(nearestWholeQ([5, 0]), 5);  // a zero denominator reads as 1
});
