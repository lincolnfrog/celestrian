...
function takePerCell(L, id, cells) {
    const out = new Array(cells).fill(null);
    for (const f of L.frames) {
        const cell = Math.floor(f.phase / L.quantum);
        const h = f.heard.find(x => x.id === id);
        if (h && out[cell] === null && f.phase % L.quantum > L.frame && f.phase % L.quantum < L.quantum - L.frame)
            out[cell] = h.take;
    }
    return out;
}
...
test('new take, select, comp, cancel', async ({ page }) => {
    ...
    expect(takePerCell(L, c2, 4)).toEqual([1, 1, 1, 1]);
    await call(page, 'selectTake', c2, 0);
    L = await listen(page);
    expect(takePerCell(L, c2, 4)).toEqual([0, 0, 0, 0]);

    // The comp: cells [0, 1, 0, 1] → the takes alternate per Q.
    await call(page, 'setComp', c2, [0, 1, 0, 1]);
    L = await listen(page);
    expect(takePerCell(L, c2, 4)).toEqual([0, 1, 0, 1]);
